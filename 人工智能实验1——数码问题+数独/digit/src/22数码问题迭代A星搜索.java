    ...
    private static void try_to_solve(String s)
    {
        Queue<State> q = new PriorityQueue<>(
            ...
            public int compare(State e1,State e2) {
                return (e1.distant+2*e1.step-e2.distant-2*e2.step);
                }
            }
        );//定义优先队列的数据结构
        ...
        int flag2=0;//标志7是否放到了正确的位置上
        for(k=0;k<100000 && !q.isEmpty();k++)//到底循环求解多少次
        {
            State head=q.poll();
            String temp=head.array;
            String temp2="";
            if(flag2==0&&head.distant<=100)//7已摆到了正确位置，清空队列重新来
                {
                    va.init_d=100;
                    flag2=1;
                    while(!q.isEmpty())
                        q.poll();
                    System.out.println("7摆到正确位置所用步数"+k);
                }
            if(head.distant==0)
                {   
                va.result=1;   
       ...
       if(q.peek()!=null)
       {
        ...
        for(i=0;i<5;i++)
         {   
            {for(j=0;j<5;j++)
                System.out.printf("%3d",s2.array.charAt(i*5+j)-'a');
            }
         System.out.println("");
         }
	    }
                        ...
                        if(b[x*va.n+y]==temp[i*va.n+j])
                            {
                                ...
                                {
                                distant+=(Math.abs(x-i)+Math.abs(y-j))*100;
                                temp[i*va.n+j]=-1;
                                flag=1;
                                break;                  
								}
